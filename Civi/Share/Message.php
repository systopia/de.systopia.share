...
class Message {
  ...
  public static function createFromSerializedMessage(array $serializedMessage, ?\DateTime $receivedDate = NULL) {
    $message = new self();

    try {
      $sourceNode = ShareNode::get()
        ->addSelect('id')
  ...
  public function getChangeIds(bool $onlyPersisted = FALSE): Iterable {
    foreach ($this->changes as $change) {
      if ($onlyPersisted && !$change->isPersisted()) {
        continue;
      }
      yield $change->getId();
    }
  ...
  public function markChanges(string $status): void {
    ShareChange::update(TRUE)
      ...
      ->addWhere('id', 'IN', $this->getChangeIds(TRUE))
      ->execute();
  }
